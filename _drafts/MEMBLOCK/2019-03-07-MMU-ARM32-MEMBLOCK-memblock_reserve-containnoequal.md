---
layout: post
title:  "memblock_reserve() Reserved 插入一块包含但大于源的内存区块"
date:   2019-03-07 10:28:30 +0800
categories: [MMU]
excerpt: memblock_reserve() Reserved 插入一块包含但大于源的内存区块.
tags:
  - MMU
---
...
memblock_reserve() 函数的作用就是将一块物理内存区块加入到预留物理内存内。新加入一块
内存区块，但这块内存区块的起始地址和已存在的内存区块相等，并且新的内存区块的终止地址大
于已存在的内存区块，通过本实践确认这种情况下，memblock_reserve() 函数会合并这两个内存
区块，但合并之后的基地址和长度都与新内存区块一致。
