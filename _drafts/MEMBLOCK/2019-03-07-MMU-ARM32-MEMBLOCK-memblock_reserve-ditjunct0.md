---
layout: post
title:  "memblock_reserve() Reserved 添加一块后端不相连的内存区块"
date:   2019-03-07 10:01:30 +0800
categories: [MMU]
excerpt: memblock_reserve() Reserved 添加一块后端不相连的内存区块.
tags:
  - MMU
---
...
memblock_reserve() 函数的作用就是将一块物理内存区块加入到预留物理内存内。新加入的内存
区块与已存在的内存区块位置上不想接，并且新的内存区块位于高地址上。实践的目的是验证新加
入的内存区块会被加入到预留区的链表尾部，但不与其他内存区块合并。
